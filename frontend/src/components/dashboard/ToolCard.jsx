...
export const ToolCard = ({ tool, onDelete, onUpdate }) => {
  ...
  // Main access handler - Uses gateway for secure access
  const handleToolAccess = async () => {
    ...
    try {
      ...
      // Use gateway for secure access with credentials
      if (hasCredentials) {
        const response = await toolsAPI.startGatewaySession(tool.id);
        
        if (response.gateway_url) {
          // Open the secure gateway page
          const fullUrl = process.env.REACT_APP_BACKEND_URL + response.gateway_url;
          window.open(fullUrl, "_blank", "noopener,noreferrer");
          
          toast.success(`${tool.name} gateway opened`, {
            description: "Use the secure copy buttons to login",
            icon: <Shield className="h-4 w-4" />,
          });
        } else {
          throw new Error("Failed to start gateway session");
        }
      } else {
        // No credentials - just open the tool URL directly
        ...
        if (toolUrl && toolUrl !== "#") {
          ...
          toast.error("Tool URL not configured", {
            description: "Please contact your administrator",
          });
        }
      }
    } catch (error) {
      console.error("Tool access failed:", error);
      ...
      <Card className="border-2 border-border/50 shadow-card hover-lift group">
        <CardContent className="p-6 flex flex-col h-full">
          ...
          <div className="flex gap-2">
            {/* Secure Tool Access via Gateway */}
            <Button
              variant="gradient"
              ...
              <div className="space-y-3 pt-4 border-t">
                ...
                <div className="p-3 rounded-lg bg-warning/10 border border-warning/20 text-sm">
                  ...
                  <p className="text-muted-foreground text-xs mt-1">
                    These credentials are encrypted and stored securely. Users access tools via secure gateway without seeing passwords.
                  </p>
                </div>
