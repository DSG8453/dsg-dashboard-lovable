import { useState } from "react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { Loader2 } from "lucide-react";

// Color scheme prop to switch between themes
export const LoginPage = ({ colorScheme = "white" }) => {
  const [isLoading, setIsLoading] = useState(false);

  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      const redirectUrl = window.location.origin + '/';
      const authUrl = `https://auth.emergentagent.com/?redirect=${encodeURIComponent(redirectUrl)}`;
      window.location.replace(authUrl);
    } catch (error) {
      setIsLoading(false);
      toast.error("Login failed");
    }
  };

  // Color schemes
  const schemes = {
    white: {
      bg: "bg-gray-50",
      card: "bg-white shadow-xl",
      text: "text-gray-900",
      subtext: "text-gray-500",
      button: "bg-white border-2 border-gray-200 hover:bg-gray-50 text-gray-700",
    },
    dark: {
      bg: "bg-gray-900",
      card: "bg-gray-800 shadow-2xl",
      text: "text-white",
      subtext: "text-gray-400",
      button: "bg-gray-700 border-2 border-gray-600 hover:bg-gray-600 text-white",
    },
    blue: {
      bg: "bg-blue-50",
      card: "bg-white shadow-xl",
      text: "text-blue-900",
      subtext: "text-blue-600",
      button: "bg-white border-2 border-blue-200 hover:bg-blue-50 text-blue-700",
    },
    gradient: {
      bg: "bg-gradient-to-br from-blue-50 via-white to-indigo-50",
      card: "bg-white/90 backdrop-blur shadow-xl",
      text: "text-gray-900",
      subtext: "text-gray-500",
      button: "bg-white border-2 border-gray-200 hover:bg-gray-50 text-gray-700",
    },
  };

  const theme = schemes[colorScheme] || schemes.white;

  return (
    <div className={`min-h-screen ${theme.bg} flex items-center justify-center p-4`}>
      <div className="w-full max-w-sm">
        {/* Card */}
        <div className={`${theme.card} rounded-3xl p-8 text-center`}>
          {/* Logo */}
          <div className="mb-6">
            <img
              src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/691ee53ded166d6334e8b9c6/0583cf617_315logodsg_.png"
              alt="DSG Transport LLC"
              className="h-24 w-auto mx-auto"
            />
          </div>
          
          {/* Company Name */}
          <h1 className={`text-xl font-medium ${theme.text} mb-8`}>
            DSG transport llc
          </h1>

          {/* Google Button */}
          <Button
            variant="outline"
            className={`w-full h-12 gap-3 text-base font-medium rounded-xl ${theme.button}`}
            onClick={handleGoogleLogin}
            disabled={isLoading}
          >
            {isLoading ? (
              <>
                <Loader2 className="h-5 w-5 animate-spin" />
                Connecting...
              </>
            ) : (
              <>
                <svg className="h-5 w-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                </svg>
                Continue with Google
              </>
            )}
          </Button>

          {/* Copyright */}
          <p className={`text-xs ${theme.subtext} mt-6`}>
            © 2025 DSG Transport LLC. All rights reserved.
          </p>
        </div>
      </div>
    </div>
  );
};
